...
/*class DateTimeType extends ProtoType {
  ...
  override def create: Any = { //генерация случайных чисел
    // дата [1; 31]
    val minDay = 1
    val maxDay = 31
    // месяц [1; 12]
  ...
  override def readValue(inputStream: InputStream): Any = parseValue(new BufferedReader(new InputStreamReader(inputStream)).lines.collect(Collectors.joining("\n")))

  override def parseValue(someString: String): Any = {
